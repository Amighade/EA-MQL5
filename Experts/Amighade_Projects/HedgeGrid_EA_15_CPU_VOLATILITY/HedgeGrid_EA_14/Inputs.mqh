...
// [REV-2026-09-14-CLOSE-STYLE] why: broadened from InpCloseLosersBulk
// (which only covered the loser purge) to cover BOTH close-heavy paths --
// the loser purge at arm AND the winner-side cleanup sweep after an SL
// hit. Same reasoning applies to both: bulk risks broker rate-limiting
// (TRADE_RETCODE_TOO_MANY_REQUESTS exists for a reason) since every
// request fires in one instant with no pacing; paced avoids that by
// reacting to real confirmations one at a time. Default PACED, per the
// concluded safer choice. Enum instead of bool since "how do we close"
// is a single choice applied in more than one place, not two independent
// yes/no toggles.
enum ENUM_CLOSE_STYLE
  {
   CLOSE_STYLE_PACED,   // one-by-one, confirmation-driven (default, safer)
   CLOSE_STYLE_BULK     // one-shot burst, all at once
  };
input ENUM_CLOSE_STYLE InpCloseStyle = CLOSE_STYLE_PACED; // How to close: loser purge at arm AND winner cleanup after SL hit
input int    InpCloseStuckAlarmAfter = 3;      // Consecutive stagnant recheck cycles (~2s each) before Telegram alarm

// [REV-2026-09-15-CPU-VOLATILITY] why: the armed SL trail is the hottest
// path during fast markets. The old code recomputed the SL candidate on
// every tick, even when price had moved only a fraction of a tick or when
// several ticks arrived inside the same millisecond window. These two
// guards deliberately throttle only the EXPENSIVE trail recalculation;
// they do not throttle fills, cleanup confirmations, or initial SL arming.
// Small defaults preserve responsiveness while flattening burst CPU load.
input int    InpSLTrailMinIntervalMs = 50;  // minimum time between heavy trail calculations
input int    InpSLTrailMinMoveTicks  = 1;   // minimum favorable price movement before recalculation
