...
// [REV-2026-09-15-CPU-VOLATILITY] why: session permissions can only change
// at a minute boundary for the configured HH:MM windows. The old coordinator
// rebuilt UTC/DST/window state on every tick. Cache by minute so bursty tick
// flow does not multiply that housekeeping cost. On the first call (or after
// an EA restart) the cache is populated immediately.
bool GetCachedSessionAllowed(GridState &state)
{
   if(!UseTimeFilter)
   {
   ...
   datetime minuteBucket = (now / 60) * 60;
   if(state.lastSessionCheckMinute != minuteBucket)
   {
      state.lastSessionCheckMinute = minuteBucket;
