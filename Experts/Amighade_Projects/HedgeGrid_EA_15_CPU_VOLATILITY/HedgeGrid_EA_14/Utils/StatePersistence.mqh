...
// [REV-2026-09-14-CLOSE-STYLE] why: bumped from 1 -- new fields below
// didn't exist under version 1. LoadGridState already treats a version
// mismatch as "ignore, fresh start" (see below), so old saved files on
// disk just get discarded on first load after this update -- no
// migration needed, the existing mismatch handling covers it.
...
void SaveGridState(GridState &state)
{
   ...
   // [REV-2026-09-14-CLOSE-STYLE] why: fields added across the
   // REV-2026-09-11-CPU-FIX / -BACKBONE / -CLOSE-SAFETY passes were
   // never added here -- flagged as a known gap, now fixed.
   FileWriteDouble (fh, state.buyVolume);
   FileWriteDouble (fh, state.buyAvgEntry);
   FileWriteDouble (fh, state.sellVolume);
   FileWriteDouble (fh, state.sellAvgEntry);
   FileWriteInteger(fh, state.winnerStoppedOut);

   FileWriteLong   (fh, (long)state.lastCleanupUnstickCheck);
   FileWriteInteger(fh, state.lastCleanupRemainingCount);
   FileWriteInteger(fh, state.cleanupStuckCount);

   FileWriteInteger(fh, ArraySize(state.loserPurgeSequence));
   FileWriteArray  (fh, state.loserPurgeSequence);
   FileWriteInteger(fh, state.loserPurgeIndex);
   FileWriteInteger(fh, state.loserPurgeInProgress);
   FileWriteInteger(fh, state.lastLoserPurgeRemainingCount);
   FileWriteInteger(fh, state.loserPurgeStuckCount);
...
bool LoadGridState(GridState &state)
{
   ...
   // [REV-2026-09-14-CLOSE-STYLE] see matching write block in SaveGridState above.
   state.buyVolume          = FileReadDouble(fh);
   state.buyAvgEntry        = FileReadDouble(fh);
   state.sellVolume         = FileReadDouble(fh);
   state.sellAvgEntry       = FileReadDouble(fh);
   state.winnerStoppedOut   = (bool)FileReadInteger(fh);

   state.lastCleanupUnstickCheck   = (datetime)FileReadLong(fh);
   state.lastCleanupRemainingCount = FileReadInteger(fh);
   state.cleanupStuckCount         = FileReadInteger(fh);

   int loserPurgeCount = FileReadInteger(fh);
   ArrayResize(state.loserPurgeSequence, loserPurgeCount);
   FileReadArray(fh, state.loserPurgeSequence);
   state.loserPurgeIndex               = FileReadInteger(fh);
   state.loserPurgeInProgress          = (bool)FileReadInteger(fh);
   state.lastLoserPurgeRemainingCount  = FileReadInteger(fh);
   state.loserPurgeStuckCount          = FileReadInteger(fh);
