...
void ResetSLManager(GridState &state)
{
   ...
   // [REV-2026-09-15-CPU-VOLATILITY] why: clear the trail throttle state
   // with the SL epoch so the next armed wall gets an immediate first
   // calculation instead of inheriting timing/price data from an old wall.
...
void ArmSL(GridState &state)
{
   ...
   // [REV-2026-09-14-CLOSE-STYLE] why: InpCloseLosersBulk (bool, loser-only)
   // replaced with InpCloseStyle (enum, shared with winner cleanup -- see
   // CleanupReset.mqh StartCleanupSequence for the other half). Reasoning
   // unchanged: bulk-close was replaced with the same paced, confirmation-
   // based mechanism the winner-side cleanup already uses -- avoids the
   // broker rate-limit risk a burst of simultaneous close requests
   // carries. InpCloseStyle=BULK still allows a bulk first-pass for
   // comparison; either way, StartLoserPurgeSequence below rescans and
   // picks up whatever's still open, so the recheck path is identical
   // regardless of which mode ran first. Orders are NOT paced -- deleting
   // a resting (not yet triggered) pending order has no broker-burst risk
   // the way closing a live position does, so they stay a single bulk
   // call, unconditionally, immediately.
...
{
   // [REV-2026-09-15-CPU-VOLATILITY] why: this function used to call the
   // heavy SL candidate engine on EVERY tick while the wall was armed.
   // During volatile bursts the tick rate can multiply, so an otherwise
   // identical calculation was repeated hundreds/thousands of times.
   // The throttle is deliberately here -- after the cheap armed-state
   // checks, but BEFORE CalculateSLCandidate(). No fill/cleanup event is
   ...
   ulong interval = (InpSLTrailMinIntervalMs > 0) ? (ulong)InpSLTrailMinIntervalMs : 0;
   double tickSize = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
   if(tickSize <= 0.0) tickSize = _Point;
   int moveTicks = (InpSLTrailMinMoveTicks > 0) ? InpSLTrailMinMoveTicks : 0;
   double minMove = tickSize * moveTicks;

   bool firstRun = (state.lastSLTrailMs == 0);
   bool intervalReady = firstRun || (nowMs - state.lastSLTrailMs >= interval);
   bool priceReady = firstRun || (moveTicks == 0) || (MathAbs(price - state.lastSLTrailPrice) >= minMove);

   if(!intervalReady || !priceReady) return;

   state.lastSLTrailMs = nowMs;
   state.lastSLTrailPrice = price;
