...
//+------------------------------------------------------------------+
//| Calculate breakeven price for all positions in one direction     |
//+------------------------------------------------------------------+
double CalculateBreakeven(ENUM_POSITION_TYPE direction, int magicNumber)
{
   ...
   return AlignToTick(_Symbol, totalCost / totalLots);
}
...
//| Constraint: SL must be beyond positions relative to price        |
//+------------------------------------------------------------------+
double CalculateSLLevel(ENUM_POSITION_TYPE winningDirection, int magicNumber)
{
   double breakeven      = CalculateBreakeven(winningDirection, magicNumber);
   double lastEntryPrice = GetLastEntryPrice(winningDirection, magicNumber);
   ...
   if(winningDirection == POSITION_TYPE_BUY)
     {
      // BUY: SL below price — take higher of breakeven and last entry
     ...
     {
      // SELL: SL above price — take lower of breakeven and last entry
   ...
   return AlignToTick(_Symbol, slLevel);
}
