...
//| ENUMS                                                             |
//+------------------------------------------------------------------+
// Short lightweight struct for tracking trade events instantly
struct MqlTradeTransactionShort 
{
...
enum ENUM_GRID_ANCHOR_MODE { ANCHOR_CURRENT_PRICE = 0, ANCHOR_PREV_BAR_RANGE = 1 };
...
input double InpInitialGap    = 0.70;  // Gap between nearest BUY and SELL ($)
input double InpGridSpacing   = 0.30;  // Distance between grid levels ($)

//--- Grid level counts (consolidated — replaces old InpGridLevels)
input int    InpInitialGridLevels = 3; // Orders per side placed on a fresh grid build
input int    InpMinGridLevels     = 3; // Refill-outside trigger: refill when count drops below this
input int    InpMaxGridLevels     = 4; // Refill-outside target: refill up to this count
...
// Allow trading: London session
input bool   EnableNewYork   = true;  
// Allow trading: New York session
input bool   EnableAsia      = false; 
// Allow trading: Asia session
...
// Commission per lot round-turn ($) gold is 6.0
...
input bool   InpEnableTelegramAlerts = true; 
// Send Telegram alert on safety-stop trigger

//--- Dashboard
input bool   InpShowDashboard     = false;  
// Show dashboard panel

//--- Scheduler / timer architecture ----------------------------------
// One native high-resolution timer drives all software tiers.  The values
// below are starting points for profiling; they are not strategy parameters.
input int    InpSchedulerHeartbeatMs      = 15;   
// Native scheduler heartbeat (MQL5 real-time floor is roughly 10-16ms)
input int    InpSchedulerBudgetMs         = 15;   
// ONE global work budget shared by the complete heartbeat
input int    InpFastTierIntervalMs       = 50;   
// Fast price/strategy work release interval
input int    InpMediumTierIntervalMs     = 250;   
// Structural reconciliation / grid lifecycle interval
input int    InpBackgroundTierIntervalMs = 60000; 
// Low-frequency housekeeping interval
input int    InpSchedulerMaxTransactionsPerFrame = 32; 
// Lossless queue processing cap per heartbeat
input int    InpCleanupAsyncBatch        = 3;    
// Max simultaneous cleanup close/delete submissions
input int    InpAsyncConfirmGraceMs      = 500;  
// Minimum delay before retrying a still-existing async target
input int    InpSessionCheckIntervalMs   = 1000; 
// Session state changes only need second-level cadence

// Dashboard refresh remains independently configurable and is executed by the
// slow software tier.
input int    InpTimerIntervalSec  = 2;     
// Dashboard refresh interval (seconds)

//--- Logging
input bool   InpEnableDebugLog    = false;  
// Debug logging to Experts tab
input bool   InpEnableHistoryLog  = false;  
// History logging to CSV file
