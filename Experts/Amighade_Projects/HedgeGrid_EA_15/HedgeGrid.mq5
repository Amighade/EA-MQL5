...
void CheckAndBuildGrid(GridState &state)
{
   ...
   // Hand control entirely to the builder logic.
   // If current market conditions pass the dynamic fallback logic, the grid canvas ignites.
   BuildGrid(SymbolInfoDouble(_Symbol, SYMBOL_BID), state.lotMode, state);
}


void CheckAndBuildGrid_old(GridState &state)
{
...
{
   // 1. CONSOLIDATED GATEKEEPER ENTRY
   // Capture only the core events that shift our terminal footprint
   ...
   // ============================================================
   // PHASE 1: CLEANUP SHIELD BLOCK (Absolute priority)
   // ============================================================
   if(g_state.cleanupInProgress)
     {
      ...
      return; // Cleanup always owns the thread until it finishes
     }

   // Cache the real-time truth directly from the server memory heap
   int totalLivePositions = CountPositions(g_state.magicNumber);
   int totalLiveOrders    = CountOrders(g_state.magicNumber);
   int targetInitialCount = GetMaxLevels(g_state.lotMode) * 2;

   // ============================================================
   // PHASE 2: INITIAL GRID DEPLOYMENT TUNNEL
   // ============================================================
   // If we have 0 open positions, but our total pending orders haven't reached
   // the target layout size yet, we are factually still building the initial grid canvas.
   if(totalLivePositions == 0 && totalLiveOrders < targetInitialCount && g_state.gridPlaced && !g_state.needsGridVerification)
     {
      if(trans.type == TRADE_TRANSACTION_ORDER_ADD)
        {
         // Drive the initial grid build step-by-step using the broker's pulse stream
         BuildGrid(SymbolInfoDouble(_Symbol, SYMBOL_BID), g_state.lotMode, g_state);
        }
      return; // Hard lock: block all normal operations, shifts, and refills until build finishes
     }

   // ============================================================
   // PHASE 3: LIVE CYCLE TRADING MODE
   // ============================================================
   
   // Handle asynchronous order updates (fills/deletions/refills) while idle
   if(trans.type == TRADE_TRANSACTION_ORDER_ADD || trans.type == TRADE_TRANSACTION_ORDER_DELETE)
     {
      ...
      return; // Order adjustments exit here safely without touching position math
     }

   // From this point down, evaluate strictly for live position execution (DEAL_ADD)
