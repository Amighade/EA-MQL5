...
void BuildGrid(double price, ENUM_LOT_MODE lotMode, GridState &state)
{
   // STEP A: Initial Setup Pass — runs ONLY on the absolute first tick of a cycle build
   if(!state.gridPlaced && state.anchorBuy == 0.0 && state.anchorSell == 0.0)
     {
      double firstBuy  = 0.0;
      double firstSell = 0.0;
      bool useLiveFallback = true;
      
      // FIXED SYNTAX: Extracted halfGap to file scope pass so both anchor blocks can read it cleanly
      double halfGap = InpInitialGap / 2.0;

      // Evaluate the previous candle's boundaries if ANCHOR_PREV_BAR_RANGE is active
      if(InpGridAnchorMode == ANCHOR_PREV_BAR_RANGE)
        {
         ENUM_TIMEFRAMES tf = (Timeframe == 0) ? (ENUM_TIMEFRAMES)Period() : Timeframe;
      ...
      // If ANCHOR_CURRENT_PRICE was chosen as default input parameter:
      else 
        {
         firstBuy  = AlignToTick(_Symbol, price + halfGap);
         firstSell = AlignToTick(_Symbol, price - halfGap);
      ...
      // Initial anchors lock into memory securely
      state.anchorBuy       = firstBuy;
      state.anchorSell      = firstSell;
      state.cycleActive     = false;
      state.currentBlockLot = GetLot(1, lotMode);
      state.farthestHitBuy  = 0.0;
      state.farthestHitSell = 0.0;
      state.gridPlaced      = true; // Secure the initialization lock instantly
      LogGridBuilt(firstBuy, firstSell, lotMode==LOT_FULL?"FULL":"HALF");
     }

   // STEP B: Pulse Loop Execution Pass — (Re-derived via dynamic server counts)
   int currentBuyCount  = CountOrderType(ORDER_TYPE_BUY_STOP, state.magicNumber);
   int currentSellCount = CountOrderType(ORDER_TYPE_SELL_STOP, state.magicNumber);
   int nextLevel = MathMax(currentBuyCount, currentSellCount) + 1;
   int maxLevels = GetMaxLevels(lotMode);

   if(nextLevel > maxLevels)
     {
      state.needsGridVerification = true; // Signal completion to validation pass
      return;
     }

   // STEP C: Place exactly ONE clean pair for the target level index
   double bid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   double spread  = ask - bid;
   double minStop = MinStopDistancePrice(_Symbol);
   ...
   double range = 0.0;
   if(InpGridAnchorMode == ANCHOR_PREV_BAR_RANGE)
      range = state.anchorBuy - state.anchorSell;

   double slDist = GetFirstLevelSLDistance(range, spread, minStop);
   
   double bp  = AlignToTick(_Symbol, state.anchorBuy  + InpGridSpacing * (nextLevel-1));
...
void BuildGrid_old(double price, ENUM_LOT_MODE lotMode, GridState &state)
{
   // STEP A: Initial Setup Pass — runs ONLY on the absolute first tick of the candle open
   if(!state.gridPlaced && state.anchorBuy == 0.0 && state.anchorSell == 0.0)
     {
      double firstBuy, firstSell;
      if(InpGridAnchorMode == ANCHOR_PREV_BAR_RANGE)
      ...
      state.farthestHitBuy  = 0.0;
      state.farthestHitSell = 0.0;
      state.gridPlaced      = true; // Protects initialization from running again
      LogGridBuilt(firstBuy, firstSell, lotMode==LOT_FULL?"FULL":"HALF");
     }

   // STEP B: Pulse Loop Execution Pass — counts what is live to find the next level index
   int currentBuyCount  = CountOrderType(ORDER_TYPE_BUY_STOP, state.magicNumber);
   int currentSellCount = CountOrderType(ORDER_TYPE_SELL_STOP, state.magicNumber);
   
   // Identify the next step level index based on the maximum placed side
   int nextLevel = MathMax(currentBuyCount, currentSellCount) + 1;
   int maxLevels = GetMaxLevels(lotMode);

   // If the target layout depth is reached, initiate verification checks and exit
   if(nextLevel > maxLevels)
     {
      state.needsGridVerification = true; // Handed to OnTick safely
      return;
     }
   ...
   // Fire exactly one single pair request to the server queue on this pulse step
   PlaceOrderPair(bp, lot, sp, lot, state.magicNumber, bsl, ssl);
}
