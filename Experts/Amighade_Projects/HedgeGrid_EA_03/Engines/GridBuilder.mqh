...
//| Places InpInitialGridLevels orders per side at InpFixedLot       |
//| No shifting, no recentering — grid persists through SL hits      |
//+------------------------------------------------------------------+
void BuildGrid_C(double price, GridState &state)
{
   ...
   // Place InpInitialGridLevels orders per side (default 50)
   for(int level = 1; level <= InpInitialGridLevels; level++)
     {
      ...
      if(IsBullishCandle())
        {
         PlaceSellStop(sellPrice, InpFixedLot, state.magicNumber);
         PlaceBuyStop(buyPrice,   InpFixedLot, state.magicNumber);
        }
      else
        {
         PlaceBuyStop(buyPrice,   InpFixedLot, state.magicNumber);
   ...
   LogGridBuilt(firstBuyPrice, firstSellPrice, "STYLE_C_FIXED");
   LogDebug(StringFormat("[GridBuilder_C] Grid placed: %d per side, lot=%.2f, gap=%.2f",
                         InpInitialGridLevels, InpFixedLot, InpInitialGap));
}
