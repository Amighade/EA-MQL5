...
{
    // Compact, lossless-enough event record for the scheduler queue.
    // The full MqlTradeTransaction is NOT retained because the queue should
    // stay small; fields needed to identify fills/requests are copied now.
...
//--- Timer / scheduler
// These are starting values for the first scheduler revision. They are not
// strategy parameters; they only control when scheduled work gets serviced.
input int InpSchedulerHeartbeatMs = 15;
input int InpSchedulerFrameBudgetMs = 15;
input int InpSchedulerFastIntervalMs = 30;
input int InpSchedulerMediumIntervalMs = 250;
input int InpSchedulerSlowIntervalMs = 2000;
input int InpSchedulerBackgroundIntervalMs = 60000;
input int InpSchedulerMaxTransactionsPerFrame = 3;
input int InpCleanupAsyncBatchSize = 3;
