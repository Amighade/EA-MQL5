...
//+------------------------------------------------------------------+
void ExecuteDeinitSafetyClose(GridState &state, string reason)
  {
   ...
   for(int i = 0; i < ArraySize(ordOrder); i++)
     {
      if(DeleteOrderSync(ordOrder[i])) deletedCount++;
...
{
   // Safety stop only raises an urgent scheduler request.
   // No synchronous trade operation, Sleep(), reset, or WebRequest here.
   if(!state.safetyStopPending)
      state.safetyStopReason = reason;   // preserve first/root reason

   state.safetyStopPending  = true;
   state.safetyAlertPending = true;

   LogDebug(StringFormat("[SafetyNet] TRIGGERED. Reason=%s. Full cleanup requested.",
                         reason));
}
#endif
