...
//+------------------------------------------------------------------+
//| Validate the EXACT strategy SL against broker rules.              |
//| REV 22.06: do not clamp a requested strategy level onto the       |
//| broker minimum-stop edge. A candidate that is too close is        |
//| rejected here so the SL engine never reports a different edge     |
//| price as if the intended strategy SL had been installed.          |
//| Existing protection that is already equal/better is kept.         |
//+------------------------------------------------------------------+
double ValidateStopPrice(const ulong ticket, double candidateSL)
{
   ...
   double target = AlignToTick(sym, NormalizeDouble(candidateSL, digits));
   if(target <= 0.0)
      return 0.0;

   // One tick of room avoids sending the SL exactly on the broker edge.
   double safety = (tickSz > 0.0 ? tickSz : SymbolInfoDouble(sym, SYMBOL_POINT));

   if(posType == POSITION_TYPE_BUY)
     {
      // Existing SL already protects at least the requested wall.
      if(oldSL > 0.0 && oldSL >= target)
         return AlignToTick(sym, NormalizeDouble(oldSL, digits));

      if(target > bid - minStop - safety)
         return 0.0;
     }
...
//+------------------------------------------------------------------+
//| Calculate current basket profits from stored side aggregates     |
//| O(1): no position/order scan                                     |
//+------------------------------------------------------------------+
void CalculateBasketProfitFromAggregates(GridState &state)
{
   MqlTick tick;
   ...
   if(state.buyVolume > 0.0)
     {
      buyProfit =
         (tick.bid - state.buyAvgEntry)
         * moneyPerPrice
         * state.buyVolume;
     }

   // SELL positions close at ASK.
   if(state.sellVolume > 0.0)
     {
      sellProfit =
         (state.sellAvgEntry - tick.ask)
         * moneyPerPrice
         * state.sellVolume;
     }

   double totalProfit = buyProfit + sellProfit;
   double totalVolume = state.buyVolume + state.sellVolume;

   state.basketBuyProfit  = buyProfit;
   state.basketSellProfit = sellProfit;
   state.basketProfit     = totalProfit;

   state.basketNetProfit =
      totalProfit - (InpCommissionPerLot * totalVolume);
}

//+------------------------------------------------------------------+
//| Calculate total basket profit for all EA positions               |
//+------------------------------------------------------------------+
void CalculateBasketProfits_old(GridState &state)
{
   ...
   state.basketProfit     = totalProfit;
   state.basketNetProfit  = totalNetProfit;
   state.basketBuyProfit  = buyProfit;
   state.basketSellProfit = sellProfit;
}
