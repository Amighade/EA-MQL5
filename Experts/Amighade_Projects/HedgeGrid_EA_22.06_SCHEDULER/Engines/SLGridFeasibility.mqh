...
bool SL_BrokerOK(int winnerSide, double candidate)
{
   ...
   double tickSz  = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
   double safety  = (tickSz > 0.0 ? tickSz : SymbolInfoDouble(_Symbol, SYMBOL_POINT));

   // REV 22.06: keep one tick of room beyond the broker minimum-stop
   // boundary instead of treating the exact edge as an acceptable SL.
   if(winnerSide == POSITION_TYPE_BUY)
      return (candidate <= bid - minStop - safety);

   return (candidate >= ask + minStop + safety);
}
