...
//+------------------------------------------------------------------+
void BuildGrid(double price, GridState &state)
{
   // REV 22.5: rejected pending levels are remembered only for this grid cycle.
   ClearAsyncPendingPlacementCycleState();
   ...
   int maxLevels = InpInitialGridLevels;
   ...
   state.currentBlockLot = GetLot(1);
         ...
         for(int step = 1; step <= needed; step++)
           {
            ...
            double lot   = GetOutsideRefillLot(level, anchorLot);
...
double GetOutsideRefillLot(int level, double anchorLot)
{
   switch(InpOutsideRefillStyle)
     {
      ...
      case OUTSIDE_LADDER:   return GetLot_Ladder(level);
      case OUTSIDE_EXP:      return GetLot_EXP(level);
...
//+------------------------------------------------------------------+
//| Checked after async fresh-grid placement requests have resolved. |
//| Confirms every expected level unless that exact level was         |
//| explicitly rejected/dropped in this cycle (REV 22.5 rule).       |
//| Later ladder levels retain their original spacing/lot sequence.   |
//+------------------------------------------------------------------+
bool VerifyFreshGrid(GridState &state)
{
   int maxLevels = InpInitialGridLevels;
   ...
   for(int level = 1; level <= maxLevels; level++)
     {
      ...
      // REV 22.5: a broker-rejected level is intentionally allowed to stay
      // absent. This does not compress/re-number the ladder; later levels keep
      // the same prices and lot progression they were originally assigned.
      bool buyDropped  = (!buyFound  && IsRejectedPendingLevel(ORDER_TYPE_BUY_STOP,
                                                                expectedBuy, state.magicNumber));
      bool sellDropped = (!sellFound && IsRejectedPendingLevel(ORDER_TYPE_SELL_STOP,
                                                                expectedSell, state.magicNumber));

      if((!buyFound && !buyDropped) || (!sellFound && !sellDropped))
        {
...
void CheckAndBuildGrid(GridState &state)
{
   ...
   ulong profBuildGridStart = GetMicrosecondCount();
   BuildGrid(SymbolInfoDouble(_Symbol, SYMBOL_BID), state);
   ProfilerRecord(PROF_BUILD_GRID, profBuildGridStart);
