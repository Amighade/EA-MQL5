...
void OnTick()
{
   ...
   // Perf: this used to call CountPositions() -- a full position scan --
   // on every single tick for the whole duration cleanupInProgress is
   // true. It's a rare-miss safety net, not the normal progress path
   // (that's driven by OnTradeTransaction), so it only needs to run
   // occasionally, not every tick.
   if(g_state.cleanupInProgress &&
      TimeCurrent() - g_state.lastCleanupUnstickCheck >= 2 &&
