...
//| ENUMS                                                             |
//+------------------------------------------------------------------+
// REV 30.01: transaction records live in Models/TransactionItem.mqh.
...
input int    InpAsyncResultTimeoutMs     = 10000; // REV 30.01: cap for WAIT_RESULT; review/tune before production
input int    InpAsyncLiveConfirmTimeoutMs= 10000; // REV 30.01: cap for WAIT_CONFIRM; review/tune before production
input int    InpSessionCheckIntervalMs   = 1000; // Session state changes only need second-level cadence

//--- REV 30.01 performance diagnostics ------------------------------
// Wall-clock profiler. This is not OS CPU time; it is used to locate EA work
// and blocking broker/API calls. It does not change strategy decisions.
input bool   InpEnableCpuProfiler        = true;  // Print frame + function timing diagnostics
input int    InpCpuProfileIntervalMs     = 10000; // Reporting window (milliseconds)
