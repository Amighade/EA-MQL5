...
uint RequestPlacePending(GridState &state,
                         ...
                         int level=-1)
  {
   ...
   if(existing >= 0)
     {
      uint existingId = state.items[existing].id;
      Profiler30Record(PROF30_REQUEST_PLACE_PENDING, profStart);
      return existingId;
     }
...
bool RequestDeleteOrder(GridState &state, ulong orderTicket)
  {
   ...
   if(idx < 0)
     {
      Profiler30Record(PROF30_REQUEST_DELETE_ORDER, profStart);
      return false;
     }
...
bool RequestClosePosition(GridState &state, ulong positionTicket)
  {
   ...
   if(idx < 0)
     {
      Profiler30Record(PROF30_REQUEST_CLOSE_POSITION, profStart);
      return false;
     }
...
bool RequestModifySL(GridState &state, ulong positionTicket, double newSL)
  {
   ...
   if(idx < 0)
     {
      Profiler30Record(PROF30_REQUEST_MODIFY_SL, profStart);
      return false;
     }
...
bool RequestReplaceOrder(GridState &state,
                         ...
                         double replacementTP=0.0)
  {
   ...
   if(idx < 0)
     {
      Profiler30Record(PROF30_REQUEST_REPLACE_ORDER, profStart);
      return false;
     }
   ...
   for(int i = 0; i < ArraySize(state.items); i++)
     {
      ...
      ulong profActionStart = GetMicrosecondCount();
      SendTradeItemAction(state, i);
      Profiler30Record(PROF30_SEND_TRADE_ITEM_ACTION, profActionStart);
     }
