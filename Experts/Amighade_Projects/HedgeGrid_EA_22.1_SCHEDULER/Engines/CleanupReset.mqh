...
//+------------------------------------------------------------------+
void StartCleanupSequence(GridState &state, ENUM_CLEANUP_MODE cleanupType)
{
...
//+------------------------------------------------------------------+
//| Completion barrier.                                             |
//| CLEANUP_CLOSE_ALL requires BOTH positions == 0 AND orders == 0.  |
//| CLEANUP_CLOSE_POSITIONS intentionally leaves pending orders by   |
//| user configuration, exactly as before.                            |
//+------------------------------------------------------------------+
bool CleanupTryFinish(GridState &state)
{
   int positions = CountPositions(state.magicNumber);
   if(positions > 0)
      return false;

   if(state.cleanupType == CLEANUP_CLOSE_POSITIONS)
     {
      // Legacy strategy semantics: pending orders remain by design.
   ...
   int orders = CountOrders(state.magicNumber);
...
void ProcessCleanupWork(ulong frameStartUs, ulong budgetUs, GridState &state)
{
   ...
   // First authority check: if the account is already clean, finish immediately.
   if(CleanupTryFinish(state))
      return;
