...
## 14. REV 21 SCHEDULER ARCHITECTURE

Rev 21 introduces the first working scheduler implementation discussed after v20.00. The native MQL5 timer is one configurable heartbeat (15ms starting value), with software release tiers for fast, medium, slow and background work. A single global frame budget is shared by all tiers.

`OnTick()` stores only the latest quote. `OnTradeTransaction()` appends compact transaction facts to a lossless queue and returns. The scheduler later consumes those events and runs the existing normal-fill/close-trigger logic.

Normal cleanup uses asynchronous close/delete submission in small batches. Outstanding requests may wait for broker confirmation while other scheduler work continues. Cleanup is not complete until live account state satisfies the relevant cleanup rule; for `CLEANUP_CLOSE_ALL`, that means zero EA positions and zero EA pending orders.

The scheduler intervals and batch sizes are starting values and are expected to be tuned after CPU/VPS measurements. Strategy parameters are not being redefined by the scheduler.

See `CHANGELOG_REV21_SCHEDULER.md` for the detailed change record.
