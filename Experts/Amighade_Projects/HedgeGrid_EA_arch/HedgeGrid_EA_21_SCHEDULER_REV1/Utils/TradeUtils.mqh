...
struct TradeActionResult
{
   ...
   int    lastError;
   ulong  order;
   ulong  requestId; // Async request identifier for OnTradeTransaction correlation
};
...
//+------------------------------------------------------------------+
//| Asynchronous pending-order deletion                              |
//| Used by the scheduler cleanup worker. It sends one request and  |
//| returns immediately; completion is verified later from account  |
//| state / trade transactions.                                     |
//+------------------------------------------------------------------+
TradeActionResult DeleteOrderAsync(ulong ticket, ulong &requestId)
{
   TradeActionResult out = {};
   requestId = 0;
...
//+------------------------------------------------------------------+
//| Asynchronous full-position close                                 |
//| Sends one close request and returns immediately. The caller must |
//| NOT interpret a successful send as a completed close.            |
//+------------------------------------------------------------------+
TradeActionResult ClosePositionAsync(ulong ticket, ulong &requestId)
{
   TradeActionResult out = {};
   requestId = 0;
