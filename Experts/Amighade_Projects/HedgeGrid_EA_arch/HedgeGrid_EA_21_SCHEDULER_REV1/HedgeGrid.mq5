//+------------------------------------------------------------------+
//| HedgeGrid.mq5                                                     |
//| Main EA coordinator — "brick" architecture (v21.00)            |
//| Rules:                                                           |
...
// TimerEngine is included after the coordinator-level CheckAndBuildGrid()
// definition so the scheduler can call that coordinator function cleanly.
...
int OnInit()
{
   ...
   // Rev 21 scheduler: one native heartbeat drives all software tiers.
   // The heartbeat is intentionally small; actual task intervals are separate.
   int heartbeatMs = InpSchedulerHeartbeatMs;
   if(heartbeatMs < 10) heartbeatMs = 10; // MQL5 real-time timer floor is approximately 10-16ms.
   EventSetMillisecondTimer(heartbeatMs);

   // Scheduler state is initialized explicitly here instead of ResetGridState.
   // Trading-cycle resets must not disturb the scheduler or transaction queue.
   ulong currentMs = GetTickCount64();
   g_state.g_lastTimeFast       = currentMs;
   g_state.g_lastTimeMedium     = currentMs;
   g_state.g_lastTimeSlow       = currentMs;
   g_state.g_lastTimeBackground = currentMs;
   g_state.g_txReadIndex        = 0;
   g_state.g_txDirty            = false;
   ArrayFree(g_state.g_txQueue);
   ArrayResize(g_state.cleanupPending, 0);
   g_state.latestBid            = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   g_state.latestAsk            = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   g_state.latestTickTime       = TimeCurrent();
   g_state.latestTickSequence   = 0;
   g_state.marketDirty          = false;
...
void OnTick()
{
   // Rev 21 rule: price data is LOSSY by design. We overwrite the latest
   // quote instead of queuing every tick. Heavy strategy work is scheduled.
   g_state.latestBid = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   g_state.latestAsk = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   g_state.latestTickTime = TimeCurrent();
   g_state.latestTickSequence++;
   g_state.marketDirty = true;
}
//+------------------------------------------------------------------+
//| OnTradeTransaction                                                |
//| Bug fixes applied:                                                 |
...
{
   // Rev 21 rule: this callback is an event bridge, not a strategy engine.
   // Copy the compact facts immediately, queue them, and return. The timer
   // consumes the queue under its global work budget.
   MqlTradeTransactionShort element;
   element.type       = trans.type;
   element.deal       = trans.deal;
   element.order      = trans.order;
   element.position   = trans.position;
   element.volume     = trans.volume;
   element.price      = trans.price;
   element.deal_type  = trans.deal_type;
   element.symbol     = trans.symbol;
   element.request_id = result.request_id;
   element.retcode    = result.retcode;

   int size = ArraySize(g_state.g_txQueue);
   if(ArrayResize(g_state.g_txQueue, size + 1) > size)
     {
      g_state.g_txQueue[size] = element;
      g_state.g_txDirty = true;
     }
}
...
void OnTimer()
{
   // One native timer drives every software tier. Each worker carries its own
   // continuation state, while RunScheduler enforces one global frame budget.
