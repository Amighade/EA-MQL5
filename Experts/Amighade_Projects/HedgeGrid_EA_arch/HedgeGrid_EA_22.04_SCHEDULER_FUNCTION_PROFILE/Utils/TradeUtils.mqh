      ...
      for(int attempt = 1; attempt <= 3; attempt++)
        {
         ...
         if(out.success)
           {
            ProfilerRecord(PROF_PLACE_PENDING_WIDENING, profFunctionStart);
            return out.order;
           }
           /*{
         ...
         if(action == RETRY_SAME && attempt < 3)
           {
            ulong profSleepStart = GetMicrosecondCount();
            Sleep(50 * attempt);
            ProfilerRecord(PROF_WIDEN_SLEEP, profSleepStart);
            continue;
           }
...
bool DeleteOrder(ulong ticket)
{
   ...
   if(!sent || (res.retcode != TRADE_RETCODE_DONE && res.retcode != TRADE_RETCODE_PLACED))
     {
      LogDebug(StringFormat("DeleteOrder FAILED: ticket=%I64u rc=%d err=%d", ticket, res.retcode, sendLastError));
      ProfilerRecord(PROF_DELETE_ORDER, profFunctionStart);
      return false;
     }
...
bool ModifyPositionSL(ulong ticket, double newSL)
{
   ...
   // REV 22.3: preserve the existing bounded retry count, but never Sleep()
   // inside the scheduler SL path. A retry remains synchronous, without an
   // artificial delay that blocks all EA event processing.
   for(int attempt = 1; attempt <= InpSafetyRetryAttempts; attempt++)
     {
      ...
      SendFailAction action = ClassifySendFailure(sent, res, sendLastError);
      if(action == RETRY_SAME && attempt < InpSafetyRetryAttempts)
         continue;
      break;
     }
