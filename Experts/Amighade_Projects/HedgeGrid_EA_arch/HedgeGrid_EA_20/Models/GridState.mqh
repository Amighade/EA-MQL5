...
struct GridState
  {
   ...
   MqlTradeTransactionShort g_txQueue[];       // Lossless transaction queue
   bool g_txDirty; // Flag indicating transactions need processing
   
   uint g_lastTime_50ms;
   uint g_lastTime_500ms;
   uint g_lastTime_2000ms;
   uint g_lastTime_60000ms;
   
   int g_nextCleanupIndex; 
   bool g_cleanupInProgress;
...
void ResetGridState(GridState &state)
  {
   ...
   state.g_lastTime_50ms       = 0;
   state.g_lastTime_500ms      = 0;
   state.g_lastTime_2000ms     = 0;
   state.g_lastTime_60000ms    = 0;
