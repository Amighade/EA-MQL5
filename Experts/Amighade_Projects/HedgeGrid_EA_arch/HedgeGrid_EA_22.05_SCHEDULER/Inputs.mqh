...
input int    InpMaxGridLevels     = 5; // Refill-outside target: refill up to this count
...
input bool   InpEnableTelegramAlerts = false; // Send Telegram alert on safety-stop trigger

//--- Dashboard
input bool   InpShowDashboard     = false;  // Show dashboard panel
...
input int    InpSchedulerHeartbeatMs      = 15;   // Native scheduler heartbeat (MQL5 real-time floor is roughly 10-16ms)
input int    InpSchedulerBudgetMs         = 15;   // ONE global work budget shared by the complete heartbeat
input int    InpFastTierIntervalMs       = 50;   // Fast price/strategy work release interval
input int    InpMediumTierIntervalMs     = 250;   // Structural reconciliation / grid lifecycle interval
input int    InpBackgroundTierIntervalMs = 60000; // Low-frequency housekeeping interval
input int    InpSchedulerMaxTransactionsPerFrame = 32; // Lossless queue processing cap per heartbeat
input int    InpCleanupAsyncBatch        = 3;    // Max simultaneous cleanup close/delete submissions
input int    InpAsyncConfirmGraceMs      = 500;  // Minimum delay before retrying a still-existing async target
input int    InpSessionCheckIntervalMs   = 1000; // Session state changes only need second-level cadence

// Dashboard refresh remains independently configurable and is executed by the
// slow software tier.
input int    InpTimerIntervalSec  = 2;     // Dashboard refresh interval (seconds)

//--- Logging
input bool   InpEnableDebugLog    = false;  // Debug logging to Experts tab
input bool   InpEnableHistoryLog  = false;  // History logging to CSV file
