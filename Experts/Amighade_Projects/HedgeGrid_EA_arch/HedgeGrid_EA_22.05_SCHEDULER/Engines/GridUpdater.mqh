...
//+------------------------------------------------------------------+
//| REV 22.5 async delete -> replace queue.                           |
//| The replacement is not submitted until the old pending ticket is |
//| actually gone, preserving the original delete-before-place order |
//| without blocking the scheduler on a synchronous OrderSend().      |
//+------------------------------------------------------------------+
struct PendingGridReplacement
{
   ulong           oldTicket;
...
PendingGridReplacement g_pendingGridReplacements[];

void RemovePendingGridReplacement(int index)
{
   int last = ArraySize(g_pendingGridReplacements) - 1;
   if(index < 0 || index > last) return;
   if(index != last)
     {
      g_pendingGridReplacements[index].oldTicket   = g_pendingGridReplacements[last].oldTicket;
      g_pendingGridReplacements[index].orderType   = g_pendingGridReplacements[last].orderType;
      g_pendingGridReplacements[index].price       = g_pendingGridReplacements[last].price;
      g_pendingGridReplacements[index].lot         = g_pendingGridReplacements[last].lot;
      g_pendingGridReplacements[index].magicNumber = g_pendingGridReplacements[last].magicNumber;
     }
   ArrayResize(g_pendingGridReplacements, last);
}

bool QueueGridOrderReplacement(ulong oldTicket, ENUM_ORDER_TYPE orderType,
                               double price, double lot, int magicNumber)
{
   for(int i = 0; i < ArraySize(g_pendingGridReplacements); i++)
      if(g_pendingGridReplacements[i].oldTicket == oldTicket)
         return true;

   // If the old order disappeared before this stage, place immediately.
   if(!OrderSelect(oldTicket))
     {
      if(orderType == ORDER_TYPE_SELL_STOP)
         PlaceSellStop(price, lot, magicNumber);
      else
         PlaceBuyStop(price, lot, magicNumber);
      return true;
     }

   if(!DeleteOrder(oldTicket))
      return false;

   int n = ArraySize(g_pendingGridReplacements);
   ArrayResize(g_pendingGridReplacements, n + 1);
   g_pendingGridReplacements[n].oldTicket   = oldTicket;
   g_pendingGridReplacements[n].orderType   = orderType;
   g_pendingGridReplacements[n].price       = price;
   g_pendingGridReplacements[n].lot         = lot;
   g_pendingGridReplacements[n].magicNumber = magicNumber;
   return true;
}

void ProcessPendingGridReplacements(GridState &state)
{
   ulong profFunctionStart = GetMicrosecondCount();

   if(state.cleanupInProgress)
     {
      ProfilerRecord(PROF_PENDING_GRID_REPLACEMENTS, profFunctionStart);
      return;
     }

   for(int i = ArraySize(g_pendingGridReplacements) - 1; i >= 0; i--)
     {
      if(OrderSelect(g_pendingGridReplacements[i].oldTicket))
         continue; // async delete not confirmed in live state yet

      if(g_pendingGridReplacements[i].orderType == ORDER_TYPE_SELL_STOP)
         PlaceSellStop(g_pendingGridReplacements[i].price,
                       g_pendingGridReplacements[i].lot,
                       g_pendingGridReplacements[i].magicNumber);
      else
         PlaceBuyStop(g_pendingGridReplacements[i].price,
                      g_pendingGridReplacements[i].lot,
                      g_pendingGridReplacements[i].magicNumber);

      RemovePendingGridReplacement(i);
     }

   ProfilerRecord(PROF_PENDING_GRID_REPLACEMENTS, profFunctionStart);
}

void ClearPendingGridReplacements()
{
   ArrayResize(g_pendingGridReplacements, 0);
}
   ...
   for(int i = 0; i < count; i++)
     {
      if(lots[i] < newLot)
        {
         oldLot = lots[i];
         if(QueueGridOrderReplacement(tickets[i], oppositeType,
                                      prices[i], newLot, state.magicNumber))
            updatedCount++;
        }
