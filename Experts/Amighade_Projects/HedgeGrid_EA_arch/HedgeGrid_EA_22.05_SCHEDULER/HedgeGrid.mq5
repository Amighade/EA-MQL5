...
//+------------------------------------------------------------------+
//| OnTradeTransaction                                                |
//| REV 21: this callback is an event recorder only. It performs no   |
//| history lookup, no position scan, no SL calculation, no cleanup   |
//| action and no grid mutation. This keeps the terminal event queue  |
//| service path as short as possible.                                |
//+------------------------------------------------------------------+
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        const MqlTradeRequest     &request,
                        const MqlTradeResult      &result)
{
   // REV 22.5: normal pending placement, deletion and SL modification are
   // asynchronous too. REQUEST events for this EA must therefore reach the
   // scheduler even when cleanup is not active so request_id results can be
   // correlated without doing strategy work in this callback.
   if(trans.type == TRADE_TRANSACTION_REQUEST)
     ...
     {
      // Other transaction types are relevant only for this chart symbol.
