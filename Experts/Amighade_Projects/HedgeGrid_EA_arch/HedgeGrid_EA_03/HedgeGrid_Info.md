...
## 12. STYLE C SPECIFICATION

### Overview
Style C is a persistent fixed-lot grid that survives SL hits.
Unlike Style A/B which restart fully after cleanup, Style C keeps
all pending orders after SL hit and only refills where needed.
No recentering. No grid shifting. Grid is self-maintaining.

### Key Differences from Style A/B
```
Style A/B:  SL hit → close all positions AND orders → rebuild from scratch
Style C:    SL hit → close positions only → keep orders → refill if needed
```

### Inputs (Style C specific)
```
InpInitialGridLevels = 50   // orders per side on initial placement and refill target
InpMinGridLevels     = 20   // refill trigger: if either side drops below this after SL hit
// Shared with all styles: InpFixedLot, InpInitialGap, InpGridSpacing
```
...
#### Initial Setup
```
Place InpInitialGridLevels BUY STOP orders above price
Place InpInitialGridLevels SELL STOP orders below price
All at InpFixedLot
Spacing = InpGridSpacing between each level
Gap between nearest BUY and SELL = InpInitialGap
```

#### When Order is Hit (becomes position)
```
No grid shifting
No lot updates
No recentering
Grid gap widens naturally as orders become positions
```

#### SL Trigger — ABWCL Style Arming
```
When SL trigger conditions met:
  → Evaluate winners vs losers (ABWCL math from ArmingEngine)
  → Apply SL wall to winner positions
  → Check if losers can be closed safely (net math positive)
  → If yes: close losers immediately
  → On each trail tick: re-check loser closing condition
  → Trail wall upward/downward as price moves
```

#### On SL Hit
```
1. Positions close (by SL)
2. Keep ALL existing pending orders
3. Check refill condition:
   If BUY STOP count < InpMinGridLevels OR SELL STOP count < InpMinGridLevels
   → Trigger refill on BOTH sides to InpInitialGridLevels
   Otherwise → do nothing (avoid hysteresis)
```

#### Refill Logic
```
Gap between nearest BUY STOP and nearest SELL STOP:
  Always >= InpInitialGap by definition
  (at least one order was hit before SL, widening the gap)

Inside fill (between two sides):
  Place orders from nearest SELL STOP upward toward price
  Place orders from nearest BUY STOP downward toward price
  Stop before violating InpInitialGap between nearest pair
  Skip silently on broker rejection (spread spike etc.)

Outside fill (beyond outermost orders):
  Extend BUY STOP grid above highest existing BUY STOP
  Extend SELL STOP grid below lowest existing SELL STOP
  Skip silently on broker rejection

All refill orders: InpFixedLot, InpGridSpacing spacing
```

#### Gap Maintenance
```
Self-regulating — no active enforcement needed
Gap only widens when orders are hit (become positions)
After SL hit and refill, gap restores naturally
If broker rejects inside fill order: skip and continue
```

### No Recentering, No Shifting
```
Style C does not use:
  - Recentering engine
  - ShiftingEngine
  - GridUpdater (no lot changes)
  - PassCounter logic
These are Style A/B only features.
```

### Files Added for Style C
```
Engines/GridBuilder.mqh    → BuildGrid_C() added
Engines/StyleC/
  GridFiller_C.mqh         → CheckAndRefill(), FillBuySide(), FillSellSide()
  SLManager_C.mqh          → ABWCL-style arming for Style C
  CleanupReset_C.mqh       → SL hit handler: close positions, keep orders, trigger refill
Inputs.mqh                 → InpInitialGridLevels, InpMinGridLevels added
```

---

*Document version: 1.3*
*Status: Style C specification complete — implementation in progress*
