...
void OnTick()
  {
   ...
   // EA decision: always finish current cycle, never force-close on session end
   // (per spec: "mid-cycle always continues until cycle ends by rules")
   ...
   // Recentering engine handles Style C internally (returns false immediately)
   ...
   // Check SL trigger on every tick (routes internally by style)
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult      &result)
  {
   ...
   if(g_state.cleanupInProgress)
     {
      if(trans.type == TRADE_TRANSACTION_DEAL_ADD)
        {
         bool done = ExecuteNextCloseStep(g_state); // routes by style internally

         if(done && InpStrategyStyle != STYLE_C)
           {
            // Style A/B: full reset and rebuild. Style C handles its own
            // reset + refill inside ExecuteNextCloseStep_C
            ResetCycle(g_state);
            ClearState();
            if(g_state.sessionAllowed)
              {
   ...
   //--- Update opposite grid lot sizes (skips internally for Style C)
   UpdateOppositeGrid(g_state);

   //--- Shift opposite grid to new anchor (skips internally for Style C)
   ShiftGrid(g_state);

   //--- Check SL trigger after fill (routes internally by style)
   ProcessSLManager(g_state);
   ...
   if(emergencyPressed)
     {
      ...
      // ExecuteEmergencyClose routes Style C internally
      ExecuteEmergencyClose(g_state);
      // Style A/B only: reset and rebuild
