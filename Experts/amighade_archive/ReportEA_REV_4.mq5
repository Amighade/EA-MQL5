...
// Used only when InpTodayOnly = false
input datetime        InpReportFrom        = D'2026.06.01 00:00';
input datetime        InpReportTo          = D'2026.06.06 00:00';
...
ReportPeriod GetFromTime()
{
   ...
   if(InpDaysBack > 0)
   {
      period.from = todayStart - InpDaysBack * 86400;
      period.to   = todayStart;
      return period;
   }

   period.from = InpReportFrom;
   period.to   = InpReportTo;

   if(period.to <= period.from)
   {
      PrintFormat("[REPORT] Invalid period: from=%s to=%s",
                  TimeToString(period.from, TIME_DATE | TIME_MINUTES),
                  TimeToString(period.to, TIME_DATE | TIME_MINUTES));
...
{
   ReportPeriod period = GetFromTime();
   
   datetime fromTime = period.from;
   datetime toTime   = period.to;
   ...
   for(int i = 0; i < totalDeals; i++)
   {
      ...
      // Lot usage should match closed history rows, not both open + close deals.
      ENUM_DEAL_ENTRY entry = (ENUM_DEAL_ENTRY)HistoryDealGetInteger(dealTicket, DEAL_ENTRY);
      bool isTradeDeal = (type == DEAL_TYPE_BUY || type == DEAL_TYPE_SELL);
      bool isClosingDeal = (entry == DEAL_ENTRY_OUT ||
                            entry == DEAL_ENTRY_OUT_BY ||
                            entry == DEAL_ENTRY_INOUT);
      
      if(isTradeDeal && isClosingDeal)
